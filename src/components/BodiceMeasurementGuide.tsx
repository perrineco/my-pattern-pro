          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="relative w-full">
                ...
                <div
                  className={`absolute inset-0 transition-opacity duration-200 ${view === "front" ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                >
                  <SharedBodyDiagram
                    category={category}
                    renderOverlay={(pos) => (
                ...
                <div
                  className={`absolute inset-0 transition-opacity duration-200 ${view === "back" ? "opacity-100" : "opacity-0 pointer-events-none"}`}
                >
                  <BackBodyDiagram
                    category={category}
            ...
            <div className="space-y-4">
              <p className="text-xs text-muted-foreground italic mb-2">{t("guide.clickToSwitch")}</p>
              <MeasurementInstruction
                ref={(el) => {
                  instructionRefs.current[1] = el;
                }}
                number={1}
                name={t("guide.bust")}
                description={t("guide.desc.bust")}
                highlighted={highlightedNumber === 1}
                view="front"
                onClick={() => handleInstructionClick(1)}
              />
              <MeasurementInstruction
                ref={(el) => {
                  instructionRefs.current[2] = el;
                }}
                number={2}
                name={t("guide.neckline")}
                description={t("guide.desc.neckline")}
                highlighted={highlightedNumber === 2}
                view="front"
                onClick={() => handleInstructionClick(2)}
              />
              <MeasurementInstruction
                ref={(el) => {
                  instructionRefs.current[3] = el;
                }}
                number={3}
                name={t("guide.shoulderLength")}
                description={t("guide.desc.shoulderLength")}
                highlighted={highlightedNumber === 3}
                view="front"
                onClick={() => handleInstructionClick(3)}
              />
              <MeasurementInstruction
                ref={(el) => {
                  instructionRefs.current[4] = el;
                }}
                number={4}
                name={t("guide.backWidth")}
                description={t("guide.desc.backWidth")}
                highlighted={highlightedNumber === 4}
                view="back"
                onClick={() => handleInstructionClick(4)}
              />
              <MeasurementInstruction
                ref={(el) => {
                  instructionRefs.current[5] = el;
                }}
                number={5}
                name={t("guide.backLength")}
                description={t("guide.desc.backLength")}
                highlighted={highlightedNumber === 5}
                view="back"
                onClick={() => handleInstructionClick(5)}
              />
            </div>
          </div>
...
function BodiceOverlayFront({
  ...
  return (
    <>
      {/* 1: Bust line */}
      <line
        x1={cx - pos.bustWidth / 2 - 10}
        y1={pos.bustY}
        x2={cx + pos.bustWidth / 2 + 10}
        y2={pos.bustY}
      ...
      <path
        d={`M ${cx - 12} ${pos.neckBaseY} A 12 6 0 0 0 ${cx + 12} ${pos.neckBaseY}`}
        fill="none"
        stroke="hsl(var(--foreground))"
        strokeWidth="1"
        strokeDasharray="3,2"
      ...
      <g onClick={() => onNumberClick(3)} className="cursor-pointer">
        <circle
          ...
          cy={(pos.neckBaseY + pos.shoulderY) / 2}
          r="7"
          fill={getCircleFill(3)}
        />
        <text
          x={(cx - 8 + pos.leftShoulderX) / 2}
          y={(pos.neckBaseY + pos.shoulderY) / 2 + 3}
          textAnchor="middle"
          className="fill-white text-[9px] font-bold pointer-events-none"
        >
          3
      ...
      <line
        x1={cx - pos.waistWidth / 2 - 10}
        y1={pos.waistY}
        x2={cx + pos.waistWidth / 2 + 10}
        y2={pos.waistY}
...
function BodiceOverlayBack({
  ...
  return (
    <>
      ...
      {/* 4: Back width */}
      <line
        x1={cx - pos.bustWidth / 2 + 20}
        y1={pos.backWidthY}
        x2={cx + pos.bustWidth / 2 - 20}
        y2={pos.backWidthY}
        stroke="hsl(var(--foreground))"
        strokeWidth="1.2"
      />
      ...
      <g onClick={() => onNumberClick(5)} className="cursor-pointer">
        ...
        <text
          x={cx + 30}
          y={(pos.neckBaseY + pos.waistY) / 2 + 3}
          textAnchor="middle"
          className="fill-white text-[9px] font-bold pointer-events-none"
        >
          5
...
const MeasurementInstruction = React.forwardRef<HTMLDivElement, MeasurementInstructionProps>(
  ({ number, name, description, highlighted, view, onClick }, ref) => {
    const highlightColor = "#f97316";
    ...
    return (
      ...
      >
        <div
          className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-primary-foreground bg-foreground"
